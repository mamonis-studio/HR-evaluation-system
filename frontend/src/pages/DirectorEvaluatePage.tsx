...
export default function DirectorEvaluatePage() {
  ...
  const loadPending = () => {
    setLoading(true);
    evaluationApi.directorPending().then(({ data }) => {
      setEvaluations(data);
      setLoading(false);
  ...
  const handleSubmit = async () => {
    if (!selected || !grade.trim()) {
      setMessage('評価グレードを入力してください');
      return;
    }
    setSaving(true);
    try {
      await evaluationApi.directorEvaluate(selected.id, grade, comment);
      setMessage('評価を送信しました。最終確認をお願いします。');
      setSelected(null);
      loadPending();
    } catch {
      ...
      <div>
        ...
        <Card className="p-6 mb-6">
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div>
              <p className="text-sm text-gray-500">評価者 → 管理者</p>
              <p className="font-medium">{selected.evaluatorName ?? '-'} → {selected.managerName ?? '-'}</p>
            </div>
          </div>
        ...
        {/* 役員評価フォーム */}
        <Card className="p-6">
          <h2 className="font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4">役員評価</h2>
          ...
          <div className="flex gap-3">
            ...
            <Button
              onClick={() => { if (confirm('評価を送信しますか？')) handleSubmit(); }}
              loading={saving}
              className="flex-1"
            >
              評価を送信
  ...
  return (
    <div>
      <PageHeader title="役員評価入力" description="管理者承認済みの評価を入力します" />
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">管理者評価</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {evaluations.map((e) => (
                  <tr key={e.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm"><GradeBadge grade={e.managerGrade ?? e.evaluatorGrade} /></td>
